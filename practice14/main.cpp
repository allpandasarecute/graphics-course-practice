...
int main() try {
	...
	for (int i = 0; i < input_model.meshes.size(); ++i) {
		...
		auto setup_attribute = [](int index,
								  gltf_model::accessor const &accessor) {
	...
	while (running) {
		...
		if (button_down[SDLK_w]) {
			camera_move_forward -= dt;
		}
		if (button_down[SDLK_s]) {
			camera_move_forward += dt;
		}
		if (button_down[SDLK_a]) {
			camera_move_sideways -= dt;
		}
		...
		camera_position +=
			camera_move_forward * glm::vec3(-std::sin(camera_rotation), 0.f,
											std::cos(camera_rotation));
		camera_position +=
			camera_move_sideways * glm::vec3(std::cos(camera_rotation), 0.f,
		...
		glBeginQuery(GL_TIME_ELAPSED,
					 objects_ids_query[first_free_query_object_index]);
		...
		for (int i = 0; i < 32; i++) {
			for (int j = 0; j < 32; j++) {
				...
				aabb box = aabb(input_model.meshes[0].min + offset,
								input_model.meshes[0].max + offset);
				int lod = std::max(
					0, std::min(5, (int)std::round(
									   glm::distance(offset, camera_position) /
									   lod_const)));
		...
		for (int i = 0; i < 6; i++) {
			...
			glBufferData(GL_ARRAY_BUFFER,
						 instances[i].size() * sizeof(glm::vec3),
		...
		for (int i = 0; i < objects_ids_query.size(); i++) {
			...
			glGetQueryObjectiv(i, GL_QUERY_RESULT_AVAILABLE,
							   &query_object_data);
			...
			std::cout << "query " << objects_ids_query[i] << " : "
					  << query_object_data / 1e6f << " ms passed\n";
			std::cout << "draw "
					  << instances[0].size() + instances[1].size() +
							 instances[2].size() + instances[3].size() +
							 instances[4].size() + instances[5].size()
					  << " objects" << std::endl;
