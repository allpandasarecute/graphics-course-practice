...
int main() try {
	...
	auto msdf_vertex_shader = create_shader(
		GL_VERTEX_SHADER, read_shader("../vertex_shader.glsl").data());
	auto msdf_fragment_shader = create_shader(
		GL_FRAGMENT_SHADER, read_shader("../fragment_shader.glsl").data());
	auto msdf_program =
		create_program(msdf_vertex_shader, msdf_fragment_shader);
	...
	while (running) {
		...
		if (text_changed) {
			...
			for (char c : text) {
				...
				auto vertex_1 =
					vertex({glyph.xoffset + pen.x, glyph.yoffset + pen.y},
						   (glm::vec2{glyph.x, glyph.y} / texture_sizes));
				auto vertex_2 =
					vertex({glyph.xoffset + pen.x,
							glyph.yoffset + glyph.height + pen.y},
						   (glm::vec2{glyph.x, glyph.y + glyph.height} /
							texture_sizes));
				auto vertex_3 =
					vertex({glyph.xoffset + glyph.width + pen.x,
							glyph.yoffset + pen.y},
						   (glm::vec2{glyph.x + glyph.width, glyph.y} /
							texture_sizes));
				auto vertex_4 = vertex(
					{glyph.xoffset + glyph.width + pen.x,
					 glyph.yoffset + glyph.height + pen.y},
					(glm::vec2{glyph.x + glyph.width, glyph.y + glyph.height} /
					 texture_sizes));
		...
		transform =
			glm::scale(transform, glm::vec3({2.f / width, -2.f / height, 0.f}));
