...
int main() try {
	...
	SDL_Window *window = SDL_CreateWindow(
		"Graphics course practice 9", SDL_WINDOWPOS_CENTERED,
	...
	auto debug_vertex_shader = create_shader(
		GL_VERTEX_SHADER, read_shader("../vertex_debug.glsl").data());
	auto debug_fragment_shader = create_shader(
		GL_FRAGMENT_SHADER, read_shader("../fragment_debug.glsl").data());
	auto debug_program =
		create_program(debug_vertex_shader, debug_fragment_shader);

	GLuint debug_shadow_map_location =
		glGetUniformLocation(debug_program, "shadow_map");

	glUseProgram(debug_program);
	glUniform1i(debug_shadow_map_location, 0);

	auto shadow_vertex_shader = create_shader(
		GL_VERTEX_SHADER, read_shader("../vertex_shadow.glsl").data());
	auto shadow_fragment_shader = create_shader(
	...
	GLuint shadow_transform_location =
		glGetUniformLocation(shadow_program, "transform");
	...
	glm::vec3 center = std::accumulate(bounding_box.begin(), bounding_box.end(),
									   glm::vec3(0.f)) /
					   8.f;
	...
	glFramebufferTexture(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, shadow_map,
						 0);
	glFramebufferRenderbuffer(GL_DRAW_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
							  GL_RENDERBUFFER, shadow_render);
	...
	while (running) {
		...
		glm::vec3 light_x =
			glm::normalize(glm::cross(light_z, {0.f, 1.f, 0.f}));
		...
		glm::mat4 transform = glm::inverse(
			glm::mat4({{dx * light_x.x, dx * light_x.y, dx * light_x.z, 0.f},
		...
		glUniformMatrix4fv(shadow_transform_location, 1, GL_FALSE,
