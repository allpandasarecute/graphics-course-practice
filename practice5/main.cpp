...
int main() try {
	...
	glBufferData(GL_ARRAY_BUFFER,
				 cow.vertices.size() * sizeof(obj_data::vertex),
				 cow.vertices.data(), GL_STATIC_DRAW);

	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, cow.indices.size() * sizeof(uint32_t),
	...
	for (int i = 0; i < size; i++) {
		for (int j = 0; j < size; j++) {
			pixels[i * size + j] =
				((i + j) % 2 == 0 ? 0xFF000000u : 0xFFFFFFFFu);
		}
	}
	...
	glTexImage2D(GL_TEXTURE_2D, 1, GL_RGBA8, size / 2, size / 2, 0, GL_RGBA,
				 GL_UNSIGNED_BYTE, pixels1.data());

	std::vector<std::uint32_t> pixels2(size / 4 * size / 4, 0xFF00FF00u);
	glTexImage2D(GL_TEXTURE_2D, 2, GL_RGBA8, size / 4, size / 4, 0, GL_RGBA,
				 GL_UNSIGNED_BYTE, pixels2.data());

	std::vector<std::uint32_t> pixels3(size / 8 * size / 8, 0xFFFF0000u);
	glTexImage2D(GL_TEXTURE_2D, 3, GL_RGBA8, size / 8, size / 8, 0, GL_RGBA,
				 GL_UNSIGNED_BYTE, pixels3.data());
	...
	int x_size, y_size, n;
	unsigned char *data =
		stbi_load(cow_texture_path.c_str(), &x_size, &y_size, &n, 4);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, x_size, y_size, 0, GL_RGBA,
