...
struct particle {
	...
	particle() {
		...
		speed = glm::vec3(
			static_cast<float>(
				0.3 * std::uniform_real_distribution<float>{-0.2f, 0.2f}(seed)),
			0.7 * static_cast<float>(size_distr(seed)),
			0.3 * std::uniform_real_distribution<float>{-0.2f, 0.2f}(seed));
		angle = glm::pi<float>();
...
int main() try {
	...
	auto geometry_shader = create_shader(
		GL_GEOMETRY_SHADER, read_shader("../geometry_shader.glsl").data());
	...
	GLuint particle_texture_location =
		glGetUniformLocation(program, "particle_texture");
	GLuint color_texture_location =
	...
	glVertexAttribPointer(
		0, 3, GL_FLOAT, GL_FALSE, sizeof(particle),
	...
	glVertexAttribPointer(2, 1, GL_FLOAT, GL_FALSE, sizeof(particle),
						  reinterpret_cast<void *>(offsetof(particle, angle)));
	...
	glTexImage1D(GL_TEXTURE_1D, 0, GL_RGBA8, colors.size(), 0, GL_RGBA,
