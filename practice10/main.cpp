...
std::pair<std::vector<vertex>, std::vector<std::uint32_t>>
generate_sphere(float radius, int quality) {
	...
	for (int latitude = -quality; latitude <= quality; ++latitude) {
		for (int longitude = 0; longitude <= 4 * quality; ++longitude) {
			...
			vertex.normal = {std::cos(lat) * std::cos(lon), std::sin(lat),
							 std::cos(lat) * std::sin(lon)};
			vertex.position = vertex.normal * radius;
			vertex.tangent = {-std::cos(lat) * std::sin(lon), 0.f,
	...
	for (int latitude = 0; latitude < 2 * quality; ++latitude) {
		for (int longitude = 0; longitude < 4 * quality; ++longitude) {
			std::uint32_t i0 =
				(latitude + 0) * (4 * quality + 1) + (longitude + 0);
			std::uint32_t i1 =
				(latitude + 1) * (4 * quality + 1) + (longitude + 0);
			std::uint32_t i2 =
				(latitude + 0) * (4 * quality + 1) + (longitude + 1);
			std::uint32_t i3 =
				(latitude + 1) * (4 * quality + 1) + (longitude + 1);
...
int main() try {
	...
	GLuint normal_texture_location =
		glGetUniformLocation(program, "normal_texture");
	GLuint reflection_map_location =
		glGetUniformLocation(program, "reflection_map");

	auto environment_vertex_shader = create_shader(
		GL_VERTEX_SHADER, read_shader("../vertex_environment.glsl").data());
	auto environment_fragment_shader = create_shader(
		GL_FRAGMENT_SHADER, read_shader("../fragment_environment.glsl").data());
	auto environment_program =
		create_program(environment_vertex_shader, environment_fragment_shader);

	GLuint view_projection_inverse_location =
		glGetUniformLocation(environment_program, "view_projection_inverse");
	GLuint camera_position_env_location =
		glGetUniformLocation(environment_program, "camera_position");
	GLuint environment_map_texture_location =
		glGetUniformLocation(environment_program, "environment_map_texture");
	...
	GLuint albedo_texture =
		load_texture(project_root + "/textures/brick_albedo.jpg");
	GLuint normal_texture =
		load_texture(project_root + "/textures/brick_normal.jpg");
	GLuint environment_map =
		load_texture(project_root + "/textures/environment_map.jpg");
	...
	while (running) {
		...
		glm::mat4 model =
			glm::rotate(glm::mat4(1.f), time * 0.1f, {0.f, 1.f, 0.f});
		...
		glUniformMatrix4fv(view_projection_inverse_location, 1, GL_FALSE,
						   reinterpret_cast<float *>(&view_projection_inverse));
		glUniform3fv(camera_position_env_location, 1,
		...
		glBindVertexArray(sphere_vao);
		glDrawElements(GL_TRIANGLES, sphere_index_count, GL_UNSIGNED_INT,
					   nullptr);
